...
if (isset($_POST['submit'])) {
    ...
    if (empty($errors)) {
        //sanitized the user input data
        $first_name = mysqli_real_escape_string($connection, $_POST['first_name']);
        $last_name = mysqli_real_escape_string($connection, $_POST['last_name']);
        $instrument = mysqli_real_escape_string($connection, $_POST['instrument']);
        $age = mysqli_real_escape_string($connection, $_POST['age']);
        $email = mysqli_real_escape_string($connection, $_POST['email']);
        $password = mysqli_real_escape_string($connection, $_POST['passwords']);

        if ($password == $_POST['confirm-password']) {
            //encrypte the password
            $hashed_password = sha1($password);

            $query = "INSERT INTO music_lessons
                    (first_name, last_name, instrument, age, email, passwords)
                    VALUES('{$first_name}', '{$last_name}' ,'{$instrument}', '{$age}', '{$email}', '{$hashed_password}')";
...
<head>
    ...
    <title>Lesson Registration | Musify</title>
    <style>
        body {
    ...
    <div class="main">
        ...
        <section id="background-picture">
            <div class="background-icon-image">
                <img src="images/background/background-4.jpg" style="width: 1335px;" alt="background image 4" class="background-image" srcset="">
            </div>
        </section>

        <section id="refistration-form">
            <div class="lesson-form">
                <form action="music-lesson-registration.php" method="post">
                    <label for="first_name">First name</label>
                    <br>
                    <input type="text" name="first_name" id="Music_name" required>
                    <br><br>
                    ...
                    <br>
                    <label for="above 18">Above 18</label>
                    <input type="radio" name="age" id="age">
                    <br><br>
                    ...
                    <br>
                    <input type="password" name="confirm-password" id="con_password" required>
